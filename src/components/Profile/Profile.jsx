...
const Profile = ({ handleLogOut, handleUpdateUser, userMessage, isError, errorMessage }) => {

  const { values, setValues, errors, isValid, resetForm, handleChange, setIsValid } = useFormValidation('');
  ...
  return (
    <section className="form-page profile">
      ...
      <form className="profile__form" noValidate onSubmit={handleUpdateUser}>
        <div className={`profile__container ${update &&'profile__container_update'}`}>
        ...
        <label className="profile__label profile__label_type_email">
          <input
          className={`profile__input ${isError && !update && 'profile__input_wrong'}`}
          name="email"
          type="email"
          autoComplete="off"
          placeholder='Почта'
        ...
        <span className='profile__error'>{errors.email}</span>
          ...
          <span className={
            isError ? 'profile__message profile__message_error': 'profile__message_hidden'}>
              {errorMessage}
            </span>
          <span className=
